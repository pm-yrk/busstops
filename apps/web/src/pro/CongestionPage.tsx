        ...
        {hotspots.length > 0 ? (
          <ul className="pro-list">
            {hotspots.map((hotspot) => (
              <HotspotCard key={hotspot.segmentId} hotspot={hotspot} />
            ))}
          </ul>
        ) : (
          <EmptyState
...
function HotspotCard({ hotspot }: { hotspot: CongestionHotspot }) {
  return (
    <li className="pro-card">
      ...
      <div className="pro-card__meta">
        ...
        {hotspot.currentSeconds !== null && hotspot.typicalSeconds !== null ? (
          <span>
            {Math.round(hotspot.currentSeconds)}s now, {Math.round(hotspot.typicalSeconds)}s typical
          </span>
        ) : null}
        {hotspot.excessVehicleMinutes !== null ? (
          <span>{hotspot.excessVehicleMinutes.toFixed(1)} excess bus-minutes</span>
        ) : null}
